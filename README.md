...
Favorites, ratings, viewing history and notes remain on the device. Best Rated uses those private ratings. Picks for You uses local category interests. New Releases lists catalog additions from the last 30 days, newest first, not claims of newly released archived software. The updated client checks GitHub when opening this collection if automatic refresh is enabled; its bundled or cached catalog remains available offline. Top Free includes only documented free applications; it does not imply download popularity. Top Rising requires sourced published trend statistics; the current catalog contains no invented download or review numbers.

## Updating the catalog

Keep `store.catalogVersion` at 3 or later and `store.catalogRevision` at 10 or later. The updated client rejects older snapshots so a stale cache cannot restore the old colors or hide the bundled additions. Add a stable unique ID, a checked archive URL, `sizeBytes`, `sha1`, and nonempty application entries under `packages` with their exact names, architecture and size. Do not include zero-byte files, dependencies or split resource packages as installable apps. Set `packageName` to an actual application entry. Add the matching logo under `Assets/Catalog` and optional splash image under `Assets/Splash`; use paths beginning with `/Assets/` in JSON. The client uses packaged artwork first and falls back to these GitHub files for newer entries.

For every new app, set `addedOn` to its first catalog publication time in UTC (ISO 8601, for example `2026-09-19T16:30:00Z`). Preserve this timestamp when editing an existing entry. New Releases uses this field automatically; no separate list needs maintaining. Increment `store.catalogRevision` and publish the catalog and its artwork together.
...
## Application previews and categories

The catalog now has 21 populated categories, including Finance, Food + Drink, Health + Fitness, Business, Developer Tools, Security, Personalization and Lifestyle. Category choices are curated from product names; they do not claim to reproduce original Store classifications.

ResultPage uses an app-colored information panel and Overview, Details and Reviews tabs. Reviews and notes stay private. The optional `screenshots` array contains `image`, `caption`, `sourceUrl` and `sourceCredit`. Five original screenshots for Fresh Paint, Bing Travel, Bing Food and Drink, and Mail/People are extracted without alteration from Microsoft's [Windows 8.1 Product Guide](https://news.microsoft.com/download/presskits/windows/docs/Windows8_1ProductGuide.pdf), pages 14, 19, 25, 26 and 28. They illustrate historical Windows 8.1 interfaces, which may differ from the exact archived package version. Their source is linked in the app. Original copyright remains with Microsoft. If no screenshot is documented, the client explicitly labels the original splash screen or package artwork instead.
